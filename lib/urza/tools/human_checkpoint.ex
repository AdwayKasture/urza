defmodule Urza.Tools.HumanCheckpoint do
  ...
  @impl Oban.Worker
  def perform(%Job{args: %{"state" => state,"message" => msg},id: id,meta: %{"workflow_id" => wf,"ref" => ref}}) do
    case state do
      "pending" -> 
  ...
  def run(%{"user_id" => user_id,"job_id" => job_id,"message" => msg}) do
    ...
    PubSub.broadcast(Urza.PubSub,"notification",{"requiring workflow update for #{user_id} for #{msg}",job_id})
    {:ok,nil}
  end
  ...
  def approve(job_id) do
    {:ok,job} = Oban.update_job(job_id,fn job -> 
      args = job.args
